...
**Stand:** 2026-07-16 (Session 001, Teil 3 — Status nach der initialen Umsetzung)
...
| `moodle_local_adele` | `ralferlebach-enrol-plugin` | `main` (0.4.2) |
| `moodle-mod_adele` | `ralferlebach-enrol-plugin` | `master` (0.1.4), Übernahme aus `ralferlebach-fix-enrolment-issue` erfolgt (R-2) |
...
| A.2 | Analyse Referenz-Plugins + Codebase, Klärung F-1…F-10, Doku-Fassung 2.0, Arbeitsplan | ✅ Session 001, Teil 2 |
| A.3 | Repos befüllen: `development` pushen, CI-Erstlauf; Arbeitsbranches in beiden Forks anlegen; CI-TODO (Branch-Referenzen) auflösen | offen, kein Blocker |
...
| C.2 | Verwaltungsseite `manage.php` nach Pflichtenheft Abschnitt 6 (inkl. „verwaist"-Markierung, Ad-hoc-Task ab Schwellwert, Bestätigungsdialog) | B.3, B.4 · **Status: offen (0.1.2)** |
| C.3 | Eigene Events (`learning_path_reconciled`, `learning_path_purged`, `user_access_revoked`) | C.1, C.2 · **Status: offen (0.1.2)** |
| C.4 | Restore-Hooks: `restore_instance()` (Skip-Strategie mit Same-Course-Ausnahme), `restore_user_enrolment()` (No-op) + Backup/Restore-Test (Prüfkriterium 5) | B.1 · **Status: offen (0.1.2)** |
| C.5 | Behat-Grundlauf für die Verwaltungsseite | C.2 · **Status: offen (0.1.2)** |
...
Verbleibend: C.2–C.5 (→ `enrol_adele` 0.1.2), D.5 und D.8.
